...
public class PizzaController {
    ...
    @GetMapping(value = "/pizzas", params = {"page", "size"})
    public ResponseEntity<CollectionModel<EntityModel<PizzaDTO>>> getPizzasPageable(@RequestParam("page") int pageNo, @RequestParam("size") int pageSize) {
        var pizzas = pizzaService.findAll(pageNo, pageSize);
        return ResponseEntity.ok(CollectionModel.of(pizzas.stream().map(pizzaModelAssembler::toModel).collect(Collectors.toList())));
    }
